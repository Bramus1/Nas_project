    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for field, msgs in errors.items():
                if field == "username":
                    friendly_messages.append("Username is invalid or too short (min 5 characters).")
                elif field == "name":
                    friendly_messages.append("name is too long (max 25 characters).")
                elif field == "password1":
                    friendly_messages.append("Password is invalid or does not meet requirements.")
                elif field == "password2":
                    friendly_messages.append("Passwords do not match.")
                else:
                    friendly_messages.append(f"{field}: {msgs[0]}")

            error_text = "\\n".join(friendly_messages)
            return HttpResponse(
                f"<script>alert('Please correct the following errors:\\n{error_text}'); window.location.href='/signup';</script>"
            )

            # return HttpResponse(
            #    f"<script>alert('Please enter valid details! \\n{errors}'); window.location.href='/signup';</script>")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if action == "create":
                ...
                password = ''.join(
                    random.choices("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890", k=8))
